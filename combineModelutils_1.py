...
def map_CSVfiles1(label_file, label_dir, data_dir, map_filename, io_lock):
    abs_name = os.path.join(label_dir, label_file)
    # read label data1 for very participant
    label_data = pd.read_csv(abs_name)
    # add data_dir and file suffix .csv to Participant ID
...
def generate_map(data_dir, label_dir, test_participant):
    # 得到当前绝对路径
    ...
    # 创建map文件夹存放map文件
    map_folder = os.sep.join([current_path, 'combine_mapfiles'])
...
# 实现seedVIG_Datasets类
class seedVIG_Datasets(Dataset):
    ...
    def __init__(self, map_file, start_num, time_num, sample_num, dataset_type):
        ...
        label = pd.Series([4] * perclos.size)
        label[awake_ind] = 3
        label[drowsy_ind] = 5
        # 在map_data中加入perclos和label数据
        map_data.insert(loc=3, column=3, value=perclos)
        map_data.insert(loc=4, column=4, value=label)
        # 在map_data中删除perclos_start和perclos_end
        map_data = map_data.drop([1, 2], axis=1)
...
if __name__ == '__main__':
    ...
    label_dir = r'E:\SEED-VIG\EEG_segments\CSVPerclos'
    data_dir = r'E:\SEED-VIG\EEG_segments\CSV128Randint'
    test_participants = ['1', '2', '3', '4']  # 定义测试集被试编码
    # 将整体数据按照被试分为训练集和测试集，如文件夹里已有map文件，会先删除再产生
    generate_map(data_dir, label_dir, test_participants)

    # test EEG_Datasets
    train_map = r"C:\Users\licy0\PycharmProjects\SEEDVIG\combine_mapFiles\train_data_map.csv"

    # 开始数据的随机值,每个epock随机一次，可以保证每个epock的样本都是不同的
    start_num = random.randrange(0, 1024)
    # 以8段数据为一个样本，修改此参数需要同时修改dataset里相应的参数
    sample_num = 8
