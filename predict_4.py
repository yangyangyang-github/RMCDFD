...
from combineModelutilsclass_2 import generate_map, seedVIG_Datasets
...
if __name__ == '__main__':
    writer = SummaryWriter("/home/yuandy/code/self-super/nihe/crossdata_acc/logs4")
    ...
    test_map2 = r"/home/yuandy/code/self-super/nihe/crossdata_acc/combine_mapfiles/train_data_map.csv"
    ...
    CombNet.load_state_dict(torch.load('/home/yuandy/code/self-super/classs/class_crossdata_acc/best_combRes50_randint_lr1_bs32_acrossSub_run2.params'))
    CombNet.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        data_loader = {"source_loader": test_loader, "target_loader": train_loader}
        a_all = 0
        cls_loss_all = 0
        ...
        metric = d2l.Accumulator(2)
        CombNet.train()
        for _, (src_examples, tar_examples) in enumerate(zip(data_loader["source_loader"], data_loader["target_loader"])):
            X, y = src_examples
            ...
            X = torch.as_tensor(X, dtype=torch.float).to(device)
            X = X.to(device)
            tar_data = torch.as_tensor(tar_data, dtype=torch.float).to(device)
            tar_data = tar_data.to(device)
            y = y.to(device)
            
         
            y_hat, states = CombNet(X)
            tar_datay_hat, tar_datastates = CombNet(tar_data)

            y_hat = encoder(y_hat)
            tar_datay_hat = encoder(tar_datay_hat)

            y_hat = y_hat.view(-1, 1)
            y = y.view(y_hat.shape)
            tar_datay_hat = tar_datay_hat.view(-1,1)
            cls_lossl = loss(y_hat, y)

            a = MMD(source=y_hat, target=tar_datay_hat)
            a_all = a_all + a
            cls_loss_all = cls_lossl + cls_loss_all
            l = 0.7 * cls_lossl + a * 0.3
            l.backward()
            optimizer.step()
        ...
        if test_l[epoch] < test_l_best:
            ...
            torch.save(encoder.state_dict(), '4classbest_combRes50_randint_lr1_bs32_acrossSub_run2.params')
            torch.save(CombNet.state_dict(), '4cnnbest_combRes50_randint_lr1_bs32_acrossSub_run2.params')
    torch.save([train_l,test_l],'4loss_combRes50_randint_lr1_bs32_acrossSub_run2.data1')
    torch.save(CombNet.state_dict(), '4end_combRes50_randint_lr1_bs32_acrossSub_run2.params')
    ...
    with torch.no_grad():
        CNNnet = resnet50(classification=False)
        CombNet = Combine(CNNnet, input_size=384 * 4, device=device, batch_first=True)
        CombNet.load_state_dict(torch.load('/home/yuandy/code/self-super/nihe/crossdata_acc/4cnnbest_combRes50_randint_lr1_bs32_acrossSub_run2.params'))
        CombNet.eval()
        CombNet.to(device)
        encoder = Encoder()
        encoder.load_state_dict(torch.load('/home/yuandy/code/self-super/nihe/crossdata_acc/4classbest_combRes50_randint_lr1_bs32_acrossSub_run2.params'))
        encoder.to(device)
        encoder.eval()
